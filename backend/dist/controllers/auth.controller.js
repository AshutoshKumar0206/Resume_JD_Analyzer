...
const signup = async (req, res) => {
    try {
        ...
        const newUser = await user_model_1.User.create({ name: name, email: email, password: hashedPassword });
...
const login = async (req, res) => {
    try {
        ...
        const user = await user_model_1.User.findOne({ email: email }).select("+password");
        if (!user) {
